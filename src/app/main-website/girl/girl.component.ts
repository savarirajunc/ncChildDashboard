...
export class GirlComponent implements OnInit, OnDestroy {
  ...
  freetrialselect;
  getfreamwork;
  constructor(private title: Title, private mainWebsiteService: MainWebsiteService, private formBuilder: FormBuilder, private activatedRoute: ActivatedRoute, private router:Router,private authenService:AuthenService) {
    this.createForm();
    this.mainWebsiteService.getGradeInfo().subscribe(response =>{
      this.getfreamwork = response.data;
    });
    this.mainWebsiteService.getNcProduct({'gender': 'girl' }).subscribe(response => {
  ...
  ngOnInit() { window.scrollTo(0, 0);
   ...
   // this.country_code = localStorage.getItem('country_code');
    this.title.setTitle(CONSTANTS.PAGETITLE.GIRL);
    this.getProductData();
