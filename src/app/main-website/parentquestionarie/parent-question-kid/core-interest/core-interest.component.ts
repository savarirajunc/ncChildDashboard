...
@Component({
  selector: 'app-core-interest',
  templateUrl: './core-interest.component.html',
  styleUrls: ['./core-interest.component.css']
})
export class CoreInterestComponent implements OnInit {
    ...
    if(!senderemail){
      this.mainWebsiteService.sendHealthAnswer({'nidara_kid_profile_id':this.kid_id,'parentQuestionAnswer':this.ques[this.currentPage]}).subscribe(response=>{
        if(response.status){
          ...
          if(user_type == 'doctor'){
            let kid_count = localStorage.getItem('kid_count');
            if(kid_count > '0'){
              localStorage.setItem('kid_count',''+kid_count+1+'');
            }
            else{
              localStorage.setItem('kid_count', '1');
            }

            this.router.navigate(['doctor/parent-qus/',localStorage.getItem('parent_id')])
          }
          else{
      ...
      this.mainWebsiteService.sendHealthAnswer({'email_id':localStorage.getItem('senderEmail'),'parentQuestionAnswer':this.ques[this.currentPage]}).subscribe(response=>{
        if(response.status){
          this.mainWebsiteService.sendquestionAnswer({'email_id':localStorage.getItem('senderEmail')}).subscribe(res=>{
            if(res.status){
              this.router.navigate(['parent-question-thanks']);
            }
          })
...
ngOnInit() { window.scrollTo(0, 0);
  ...
  let senderemail = localStorage.getItem('senderEmail');
  if(!senderemail){
    this.mainWebsiteService.getParentQusInfo({'nidara_kid_profile_id':this.kid_id,'core_id':3}).subscribe(response =>{
      this.ques = response.data.core_interest;
    ...
    this.mainWebsiteService.getParentQusInfo({'grade_id':this.kid_id,'core_id':3,'email_id':localStorage.getItem('senderEmail')}).subscribe(response =>{
      this.ques = response.data.core_interest;
      for ( let i = 0; i < this.ques.length; i++) {
        this.pageArray.push(i);
      }
