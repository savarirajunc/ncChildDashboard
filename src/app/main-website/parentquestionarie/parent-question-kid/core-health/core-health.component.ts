...
@Component({
  selector: 'app-core-health',
  templateUrl: './core-health.component.html',
  styleUrls: ['./core-health.component.css']
})
export class CoreHealthComponent implements OnInit {
    ...
    onSubmit(form){
      ...
      if(!senderemail){
        this.mainWebsiteService.sendHealthAnswer({'nidara_kid_profile_id':this.kid_id,'parentQuestionAnswer':this.ques[this.currentPage]}).subscribe(response=>{
          if(response.status){
            this.router.navigate(['parent-question/parent-qus/1/core-leargning/',this.kid_id])
  ...
  ngOnInit() { window.scrollTo(0, 0);
    ...
    let senderemail = localStorage.getItem('senderEmail');
    if(!senderemail){
      this.mainWebsiteService.getParentQusInfo({'nidara_kid_profile_id':this.kid_id,'core_id':2}).subscribe(response =>{
        this.ques = response.data.core_health;
      ...
      this.mainWebsiteService.getParentQusInfo({'grade_id':this.kid_id,'core_id':2,'email_id':localStorage.getItem('senderEmail')}).subscribe(response =>{
        this.ques = response.data.core_health;
        for ( let i = 0; i < this.ques.length; i++) {
          this.pageArray.push(i);
        }
